...
---@param n number
---@param i number
local function heapify_recursive(get_value, indices, n, i)
    local largest = i
    local left = 2 * i + 1
    local right = 2 * i + 2

    if left < n and get_value(indices[left]) > get_value(indices[largest]) then
        largest = left
    end

    if right < n and get_value(indices[right]) > get_value(indices[largest]) then
        largest = right
    end

    if largest ~= i then
        swap(indices, i, largest)
        heapify_recursive(get_value, indices, n, largest)
    end
end

---@param n number
---@param i number
local function heapify_iterative(get_value, indices, n, i)
    while true do
        local largest = i
        local left = 2 * i + 1
        local right = 2 * i + 2
...
---@param n number
function hs.heap_sort_indices(get_value, n)
    ...
    for i = math.floor(n / 2) - 1, 0, -1 do
        heapify(get_value, indices, n, i)
    end

    for i = n - 1, 0, -1 do
        swap(indices, 0, i)
        heapify(get_value, indices, i, 0)
    end
