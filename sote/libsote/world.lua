...
function world:latlon_to_hex_coords(lat, lon)
	...
	local ico_defines = require("sote.libsote.icosa-defines")
	...
	local point_on_face = spherical_coordinates_double * vec3.dot(face_normal, face_vertex3) /
	vec3.dot(face_normal, spherical_coordinates_double)
	local u, v, _ = mu.barycentric_coordinates(point_on_face, face_vertex1, face_vertex2, face_vertex3)
	local hexagonal_coordinates = triangle_coords_to_hex_coords(get_triangle_coords(v, u, self.world_size),
		self.world_size)
...
function world:_check_collisions()
	local expected = 3 + 0.5 * (3 * (self.world_size - 1) ^ 2 + 3 * (self.world_size - 1) + 2) +
	3 * (self.world_size - 1)
...
function world:_check_valid_indices()
	local max_index = 3 + 0.5 * (3 * (self.world_size - 1) ^ 2 + 3 * (self.world_size - 1) + 2) +
	3 * (self.world_size - 1)
