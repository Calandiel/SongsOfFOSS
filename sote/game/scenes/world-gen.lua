...
local function log_sote(sote_msg)
  print("[libSOTE] " .. ffi.string(sote_msg))
end
...
function wg.init()
  ...
  for _, v in ipairs(sote_params) do
    ...
    ret_code = lib_sote.LIBSOTE_SetVar(err_msg, 3, desc, ffi.cast("void*", val))

    ::continue::
  end

  ret_code = lib_sote.LIBSOTE_StartTask(err_msg, sote_tasks.init_world, 0, nil)
  if ret_code ~= 0 then
    log_sote(err_msg)
  ...
  ret_code = lib_sote.LIBSOTE_WaitEndTask(err_msg)
  if ret_code ~= 0 then
    log_sote(err_msg)
    error("failed to wait for init_world task: ")
  end
  log_info("init_world task finished")
end
