    ...
    },
    {
        ...
        render_closure = function (rect, k, v)
            ...
            if ut.money_button("", REWARD_AMOUNT, rect, tooltip, can_invest) then
                if v.quest_type == "explore" then
                    player_character.realm.quests_explore[v.target] = player_character.realm.quests_explore[v.target] + REWARD_AMOUNT
                end
                if v.quest_type == "raid" then
                    player_character.realm.quests_raid[v.target] = player_character.realm.quests_raid[v.target] + REWARD_AMOUNT
                end
                if v.quest_type == "patrol" then
