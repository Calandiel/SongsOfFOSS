...
function rea.run(realm)
	...
	for _, province in pairs(realm.provinces) do
		for prod, amount in pairs(province.local_production) do
			...
			local category = DATA.trade_good_get_category(prod)
	...
	--			province.realm.resources[prod] = province.realm.resources[prod] + to_realm_stockpile
		...
		for prod, amount in pairs(province.local_consumption) do
			...
			local weight = DATA.get_use_weight(prod, use_case("calories"))
			if weight then
	...
	for trade_good, amount in pairs(realm.resources) do
		if DATA.trade_good_get_category(trade_good) == 'good' then
			local old = amount or 0
			realm.resources[trade_good] = math.max(0, old) * 0.99
		end
	end
	...
	for _, province in pairs(realm.provinces) do
		...
		for good, amount in pairs(province.local_storage) do
			...
			if category == 'good' then
				...
				if neighbor_count > 0 then
					local sharing = province.local_storage[good] * NEIGHBOURS_GOODS_SHARING
					economic_effects.change_local_stockpile(province, good, -sharing)
					local neighbor_share = sharing / neighbor_count
					for _, neighbor in pairs(province.neighbors) do
						if neighbor.realm then
							economic_effects.change_local_stockpile(neighbor, good, neighbor_share)
						end
					end
				end

				-- decay local stockpiles
				economic_effects.decay_local_stockpile(province, good)
			end
		end
	end
	...
	for _, province in pairs(realm.provinces) do
		DATA.for_each_trade_good(function(trade_good)
			local current_price = economic_values.get_local_price(province, trade_good)
			local supply = province.local_production[trade_good] or 0
			local demand = province.local_demand[trade_good] or 0
			local stockpile = province.local_storage[trade_good] or 0
