...
function EconomicEffects.buy_use(character, use, amount)
    ...
    for _, values in pairs(goods) do
        ...
        local trade_volume = (province.local_consumption[values.good] or 0) + (province.local_production[values.good] or 0) + amount
    ...
    if WORLD:does_player_see_province_news(province) then
        WORLD:emit_notification("Trader " .. character.name .. " bought " .. amount .. " " .. use .. " for " .. ut.to_fixed_point2(spendings) .. MONEY_SYMBOL)
    end
end
...
function EconomicEffects.gift_to_tribe(character, realm, amount)
	...
	realm.capitol.mood = realm.capitol.mood + amount / realm.capitol:total_population() / 100
	character.popularity[realm] = (character.popularity[realm] or 0) + amount / (realm.capitol:total_population() + 1) / 100
end
